...
def reportfault():
    ...
    if (int(motor_params[0]) > 220 and \
        ...
        int(motor_params[2]) > 220     \
       ):
        job_minute.enabled = False
    elif (int(motor_params[0]) < 5 and \
          int(motor_params[1]) < 5 and \
          int(motor_params[2]) < 5     \
          ):
        job_minute.enabled = False
    else:
...
def motorval(bot, update):
    ...
    if(mqtt_flag==0):
        ...
        bot.send_message(chat_id=update.message.chat_id, text=voltage_text, parse_mode='MARKDOWN')
        bot.send_message(chat_id=update.message.chat_id, text=current_text, parse_mode='MARKDOWN')
        bot.send_message(chat_id=update.message.chat_id, text=duration_text, parse_mode='MARKDOWN')
    else:
