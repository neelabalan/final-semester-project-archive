...
motor_params = ["999", "999", "999", "99.99", "99.99", "99.99"]
...
j = updater.job_queue
job_voltage = j.run_repeating(callback_voltage, interval=20, first=0)
job_minute = j.run_repeating(callback_duration, interval=10, first=0)
...
def reportfault():
    ...
    if (
        int(motor_params[0]) > 220
        and int(motor_params[1]) > 220
        and int(motor_params[2]) > 220
    ):
        job_minute.enabled = False
    elif (
        int(motor_params[0]) < 5
        and int(motor_params[1]) < 5
        and int(motor_params[2]) < 5
    ):
        job_minute.enabled = False
    else:
        pass
...
def setmotor_params(topic, message):
    ...
    logcsv()
    # findduration(motor_params[3], motor_params[4], motor_params[5])
...
def findduration(currenta, currentb, currentc):
    global initialvalue, time_minute, time_hour
...
def callback_duration(bot, job):
    global time_minute, alertminute
    findduration()
    if time_minute == alertminute:
...
def start(bot, update):
    start_text = (
        "Welcome to Motor Parameters Monitoring \n"
...
def motorval(bot, update):
    ...
    voltage_text = (
        "`VOLTAGE - R {0} V`\n" "`VOLTAGE - Y {1} V`\n" "`VOLTAGE - B {2} V`  "
    ).format(motor_params[0], motor_params[1], motor_params[2])

    bot.send_message(chat_id=update.message.chat_id, text="hello")
    current_text = (
        "`CURRENT - R {0} A`\n" "`CURRENT - Y {1} A`\n" "`CURRENT - B {2} A`  "
    ).format(motor_params[3], motor_params[4], motor_params[5])

    duration_text = (
        "`TOTAL TIME DURATION - {0}`\n" "`FROM - {1}`               \n" "`TO   - {2}`"
    ).format(duration[0], duration[1], duration[2])
    if mqtt_flag == 0:
        bot.send_message(
            chat_id=update.message.chat_id, text=start_text, parse_mode="MARKDOWN"
        )
        bot.send_message(
            chat_id=update.message.chat_id, text=voltage_text, parse_mode="MARKDOWN"
        )
        bot.send_message(
            chat_id=update.message.chat_id, text=current_text, parse_mode="MARKDOWN"
        )
        bot.send_message(
            chat_id=update.message.chat_id, text=duration_text, parse_mode="MARKDOWN"
        )
    else:
        bot.send_message(
            chat_id=update.message.chat_id, text=error_text, parse_mode="MARKDOWN"
        )
...
def help(bot, update):
    ...
    bot.send_message(
        chat_id=update.message.chat_id, text=help_text, parse_mode="MARKDOWN"
    )
